...
// ================== start update courses page ======================
  $page_name = " - تعديل كوبونات الخصم";
  $script = "add_courses.js";
  include 'init.php';

  require './layout/topNav.php';
  $coupon_id = $_GET['id'];
  $coupon_data = getData_with_id("coupons",$coupon_id);
  if ($_SERVER['REQUEST_METHOD'] == 'POST'){
      ...
      if(empty($formErrors)){
            update_coupon($name,$code,$value,$expire,$one_use,$_SESSION["admin_id"],$coupon_id);
        }  
      ...
      <div id="layoutSidenav_content">
          <div class="container-fluid ">
              ...
              <div class="container mainAddForm">
                  ...
                  <p class="secondParagraph text-center pb-5">من خلال هذه الصفحه يمكنك تعديل بيانات الكوبونات</p>
                  <form method="POST" action="<?php $_SERVER['PHP_SELF'] ?>"  enctype="multipart/form-data">
                        <div style="direction: rtl !important;text-align: right;" class="row m-2 pb-3">
                                ...
                                <div class="col-md-6 mt-4 col-xs-12">
                                    ...
                                    <input type="text" class="form-control <?php echo $name_error;?>" id="name" 
                                        placeholder="ادخل اسم الكوبون" value="<?php if(isset($name)){ echo $name;}else{echo $coupon_data["coupone_name"];} ?>" name="name" autocomplete="off">
                                </div>
                                <!--code-->
                                <div class="col-md-3 mt-4 col-xs-12">
                                    <label for="code">كود كوبون الخصم</label>
                                    <input type="text" value="<?php if(isset($code)){ echo $code;}else{echo $coupon_data["coupone_code"];} ?>" class="form-control <?php echo $code_error;?>" id="code" placeholder="كود الكوبون" readonly name="code" autocomplete="off">
                                </div>
                                ...
                                <div style="margin-top:30px !important" class="input-group col-md-6 m-auto col-xs-12">
                                    <input type="text" class="form-control <?php echo $value_error;?>" id="value" 
                                        placeholder="ادخل فيمة الكوبون" value="<?php if(isset($value)){ echo $value;}else{echo $coupon_data["coupone_value"];} ?>" name="value" autocomplete="off">
                      ...
                      <!--btn -> add--->
                      <button class="btn btn-primary d-block m-auto mt-5">تعديل الكوبون</button>
                  </form>
                  </div>
              </div>
        </div>
